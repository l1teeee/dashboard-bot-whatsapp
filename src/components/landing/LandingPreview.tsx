...
export function HeroOperationsPreview() {
  return (
    <div
      aria-hidden="true"
      className="relative flex h-full min-h-[560px] items-center justify-center overflow-hidden rounded-[30px] border border-ink-dark bg-lilac px-4 py-14 text-ink-dark sm:px-8 sm:py-16"
    >
      <div className="pattern-radial absolute inset-0 opacity-40" />
      <div className="absolute left-5 top-5 flex items-center gap-2 rounded-full border border-ink-dark bg-warm px-3 py-2 text-[11px] font-extrabold uppercase tracking-[.12em] neo-shadow sm:left-8 sm:top-8">
        <span className="landing-live-dot h-2 w-2 rounded-full bg-[#48B88A]" />
        Operación conectada
      </div>

      <div className="landing-float-delay absolute right-3 top-14 z-20 hidden w-44 rotate-[5deg] rounded-[22px] border border-ink-dark bg-yellow p-4 neo-shadow sm:block lg:right-8 lg:top-16 lg:w-52">
      ...
      <div className="landing-float-soft absolute left-4 top-32 z-20 hidden w-44 -rotate-[3deg] rounded-[22px] border border-ink-dark bg-cyan p-4 neo-shadow lg:block xl:left-8">
        <div className="flex items-center justify-between">
          <span className="kicker">Tiempo medio</span>
          <Timer className="h-4 w-4" />
        </div>
        <p className="font-display mt-3 text-3xl font-bold leading-none">6 MIN</p>
        <p className="mt-2 text-xs font-bold">Del chat a la cocina</p>
      </div>

      <div className="landing-float-negative absolute bottom-14 left-3 z-20 hidden w-44 -rotate-[4deg] rounded-[22px] border border-ink-dark bg-coral p-4 neo-shadow sm:block lg:bottom-16 lg:left-8 lg:w-52">
        <div className="flex items-center gap-3">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-ink-dark bg-warm">
            <Check className="h-4 w-4" />
          </span>
          <div>
            <p className="kicker">Estado</p>
            <p className="font-display text-xl font-bold leading-tight">EN PROCESO</p>
          </div>
        </div>
      </div>

      <div className="landing-float-soft-alt absolute bottom-32 right-4 z-20 hidden w-44 rotate-[4deg] rounded-[22px] border border-ink-dark bg-mint p-4 neo-shadow lg:block xl:right-8">
        <div className="flex items-center justify-between">
          <span className="kicker">Completados</span>
          <TrendingUp className="h-4 w-4" />
        </div>
        <p className="font-display mt-3 text-3xl font-bold leading-none">18 HOY</p>
        <p className="mt-2 text-xs font-bold">Sin pedidos perdidos</p>
      </div>

      <div className="landing-phone-enter relative z-10 w-full max-w-[300px] rounded-[42px] border-[3px] border-ink-dark bg-[#111110] p-2.5 shadow-[0_8px_0_#111] sm:max-w-[315px]">
        <div className="relative overflow-hidden rounded-[32px] border border-border bg-surface pb-3 text-ink">
          ...
          <div className="flex items-center justify-between px-4 pb-3 pt-10">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[11px] font-bold text-ink-soft">Buenos días, equipo</p>
                <p className="text-sm font-extrabold">Pedidos en vivo</p>
              </div>
            </div>
          ...
          <div className="mx-3 mt-3 rounded-[22px] border border-border bg-surface-raised p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="rounded-full bg-mint px-2 py-1 text-[10px] font-black text-ink-dark">EN VIVO</span>
            </div>
            <div className="space-y-2">
              {queue.map((order) => (
                <div key={order.id} className="flex items-center gap-2 rounded-2xl bg-surface p-2.5">
                  <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl text-[11px] font-black text-ink-dark ${order.color}`}>
                    #{order.id.slice(-2)}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-extrabold">{order.item}</p>
                    <p className="mt-0.5 text-[10px] font-bold text-ink-soft">{order.time}</p>
                  </div>
      ...
      <div className="overflow-hidden rounded-[23px] border border-border bg-shell sm:rounded-[27px]">
        <div className="flex min-h-14 items-center justify-between border-b border-border px-3 sm:px-5">
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <span className="hidden text-[11px] font-extrabold uppercase tracking-[.15em] text-ink-soft sm:inline">Panel / <b className="text-ink">Resumen</b></span>
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-2 rounded-full border border-border bg-surface px-3 py-2 text-[11px] font-bold sm:flex">
        ...
        <div className="grid sm:grid-cols-[58px_1fr]">
          ...
          <div className="grid gap-2.5 p-2.5 sm:grid-cols-12 sm:gap-3 sm:p-4">
            <div className="relative overflow-hidden rounded-[22px] border border-ink-dark bg-coral p-4 text-ink-dark sm:col-span-7 sm:min-h-64 sm:p-5">
              ...
              <div className="relative">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full border border-ink-dark bg-warm px-2.5 py-1.5 text-[11px] font-black">+12.4%</span>
                </div>
                <div className="mt-6 flex h-24 items-end gap-2 sm:h-28 sm:gap-3">
                  {weekBars.map((height, index) => (
                    <div key={index} className="flex h-full flex-1 flex-col justify-end gap-1">
                      ...
                      <span className="text-center text-[10px] font-black">{['L', 'M', 'X', 'J', 'V', 'S', 'D'][index]}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="relative flex h-full flex-col">
                ...
                <div className="mt-6 flex flex-1 flex-col justify-end gap-3">
                  {[
                    ...
                    <div key={label}>
                      <div className="mb-1 flex justify-between text-[11px] font-black uppercase"><span>{label}</span><span>{value}</span></div>
            ...
            <div className="rounded-[22px] border border-ink-dark bg-cyan p-4 text-ink-dark sm:col-span-4 sm:min-h-48">
              ...
              <p className="mt-1 text-xs font-bold">Esperando hace 3 min</p>
              <div className="mt-5 rounded-xl border border-ink-dark bg-warm px-3 py-2 text-[11px] font-black">2× Burger · 1× Papas</div>
            </div>
            ...
            <div className="rounded-[22px] border border-border bg-surface p-4 sm:col-span-4 sm:min-h-48">
              ...
              <div className="mt-4 space-y-2">
                <div className="rounded-xl bg-surface-raised px-3 py-2 text-[11px] font-bold">#1047 pasó a preparación</div>
                <div className="rounded-xl bg-surface-raised px-3 py-2 text-[11px] font-bold">#1045 fue completado</div>
              </div>
            </div>
          </div>
        </div>

...
type CommandItem = { id: string; type: 'nav' | 'order'; label?: string; path?: string; icon?: React.ReactNode; orderId?: number; dailyNumber?: number | null; phone?: string; status?: string; total?: number; };
...
export function CommandPalette() {
  ...
  const items = useMemo<CommandItem[]>(() => {
    ...
    const matching = orders.filter((order) => String(order.id).includes(search) || order.phone_number.includes(search)).slice(0, 6).map((order) => ({ id: `order-${order.id}`, type: 'order' as const, orderId: order.id, dailyNumber: order.daily_number, phone: order.phone_number, status: order.status, total: order.total }));
    return [...nav, ...matching];
  }, [search, orders]);

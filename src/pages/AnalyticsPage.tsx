...
import { AnalyticsSectionNavigator } from '@/components/dashboard/AnalyticsSectionNavigator';
  ...
  return (
    <>
      {isError && !orders.length ? (
        <ErrorState message={error?.message || 'Error al cargar las analíticas'} onRetry={() => refetch()} />
      ) : (

...
import { CloseBusinessDayButton } from '@/components/orders/CloseBusinessDayButton';
...
export function OrdersPage() {
  ...
  return <div className="space-y-6"><PageHeader kicker="Registro" title="Todos los pedidos" description={`Mostrando ${filtered.length} de ${orders.length} cargados · ${total} totales`} actions={<CloseBusinessDayButton />}/><OrderFilters value={filters} onChange={setFilters} onReset={() => setFilters(initial)}/>{!isLoading && !filtered.length ? <EmptyState title="Sin pedidos" description="No hay pedidos que coincidan con los filtros" action={<Button variant="secondary" onClick={() => setFilters(initial)}>Limpiar filtros</Button>}/> : <OrderList orders={filtered} onSelectOrder={selectOrder} isLoading={isLoading}/>}<OrderDetail orderId={selectedId} onClose={closeOrder}/></div>;
}
